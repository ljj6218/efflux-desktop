...
async def update_plan(plan_vo: PlanVO, plan_service: PlanCase = Depends(plan_case)):
    conversation_id =  await plan_service.update(plan=plan_vo.plan, client_id=plan_vo.client_id, generator_id=plan_vo.generator_id, agent_instance_id=plan_vo.agent_instance_id, is_update=plan_vo.is_update)
