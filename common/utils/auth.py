...
class Secret(ABC):
    ...
    @staticmethod
    def from_api_key(api_key: str) -> "ApiKeySecret":
        """
        Create an API key-based secret. Cannot be serialized.

        :param api_key:
            The API key to use for authentication.
        """
        return ApiKeySecret(_api_key=api_key)

    @staticmethod
    def from_env_var(env_vars: Union[str, List[str]], *, strict: bool = True) -> "EnvVarSecret":
        """
...
class ApiKeySecret(Secret):
    _api_key: str
    _type: SecretType = SecretType.API_KEY

    def __post_init__(self):
        super().__init__()
        assert self._type == SecretType.API_KEY
    ...
    def _to_dict(self) -> Dict[str, Any]:
        raise ValueError(
            "Cannot serialize API key-based secret. Use an alternative secret type like environment variables."
        )

    @staticmethod
    def _from_dict(_: Dict[str, Any]) -> "Secret":
        raise ValueError(
            "Cannot deserialize API key-based secret. Use an alternative secret type like environment variables."
        )
